...
constexpr int PLUS = 0;
constexpr int MINUS = 1;
// 手元の SwitchBot のアドレス
static String addrSwitchBot[2] = {"c7:4c:61:9f:32:69", "d0:c9:7b:58:81:88"};
...
static BLEAdvertisedDevice target[2];
...
class advdCallback : public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    ...
    // SwitchBot を発見
    if (addr.equalsIgnoreCase(addrSwitchBot[PLUS])) {
      Serial.printf("found plus\n");
      target[PLUS] = advertisedDevice;
      plus_found = true;
    }
    if (addr.equalsIgnoreCase(addrSwitchBot[MINUS])) {
      Serial.printf("found minus\n");
      target[MINUS] = advertisedDevice;
      minus_found = true;
    }
...
static int connectAndSendCommand(bool is_plus, int times, int delay_ms) {
  static BLEClient* pClient[2] = {BLEDevice::createClient(), BLEDevice::createClient()};
  static BLERemoteCharacteristic* pRemoteCharacteristic[2];
  static BLERemoteService* pRemoteService[2];
  static int id;
  
  id = is_plus ? 0 : 1;

  Serial.println("loop start");
  if (!pClient[id]->isConnected()) {
    if (!pClient[id]->connect(&target[id])) {
      return REMOTESERVICE_NOT_FOUND;
    } 
    Serial.println("connected");

    // 対象サービスを得る
    pRemoteService[id] = pClient[id]->getService(serviceUUID);
    if (pRemoteService[id] == nullptr) {
      return REMOTESERVICE_NOT_FOUND;
    }
    // 対象キャラクタリスティックを得る
    pRemoteCharacteristic[id] = pRemoteService[id]->getCharacteristic(notifyUUID);
    pRemoteCharacteristic[id]->registerForNotify(is_plus ? notifyCallback0 : notifyCallback1);

    pRemoteCharacteristic[id] = pRemoteService[id]->getCharacteristic(charUUID);
    if (pRemoteCharacteristic[id] == nullptr) {
      return REMOTECHARACTERISTIC_NOT_FOUND;
    }
  }
  // キャラクタリスティックに Press コマンドを書き込む
  for (int i = 0; i < times; ++i) {
    pRemoteCharacteristic[id]->writeValue(cmdPress, sizeof(cmdPress), false);
    delay(delay_ms);
  }
  /* disconnect
  // if (pClient) {
        ...
        if (key == '+' || key == '-' ) {
            ...
            if (temp_diff >= '1' && temp_diff <= '9') {
                if (canSendCommand == true) {
                    ...
                    print_result(connectAndSendCommand(key == '+', temp_diff - '0', 3000));
                }
            } else if (temp_diff == '0') {
                print_result(connectAndSendCommand(true, 1, 1000));
                print_result(connectAndSendCommand(false, 1, 1000));
            } else {
